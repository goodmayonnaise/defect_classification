...
parser.add_argument('--save-dir', metavar='DIR', help='path to save path')
...
def main():
    ...
    if args.evaluate:
        args.save_dir = ('/').join(args.resume.split('/')[:-1])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate:
        ...
        with open(args.save_dir+'/res.txt', 'w') as f:
            print(res, file=f)
        return
...
def save_checkpoint(state, is_best, dir):
    filename = dir+'/checkpoint.pth.tar'
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if not args.pretrained and epoch < warmup_epochs:
        # linear warmup (scratch 학습용)
